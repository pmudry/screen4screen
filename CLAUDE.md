...
- `build/` - `Launcher.cs` plus `Build-Launcher.ps1`, which compiles
  ...
  committed, so a clone can be double-clicked with no build step; rebuild and
  commit it whenever `Launcher.cs` or the icon changes.
...
- `screen4screen.exe` embeds the icon, so changing `assets/screen4screen.ico`
  or `build/Launcher.cs` needs a rebuild **and a fresh commit of the .exe**,
  which is tracked. Nothing else does: the window, the language dictionaries
  and the module are all read at launch. `Build-Launcher.ps1` checks and says
  so.
...
- No external modules. The only binaries tracked are the three ISC sample
  backgrounds under `wallpapers/`, the icon under `assets/`, and
  `screen4screen.exe`; keep it that way. The icon is generated by a short PIL script, kept in the
